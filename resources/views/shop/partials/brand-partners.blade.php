                ...
                <div class="swiper-wrapper items-center">
                    @if ($partnerCount > 0)
                        @foreach ($partners as $p)
                            <div class="swiper-slide flex items-center justify-center px-4 w-auto">
                                @if ($p['logo'])
                                    <img src="{{ asset($p['logo']) }}" alt="{{ $p['name'] ?? __('Partner') }}" class="max-h-12 sm:max-h-16 max-w-full w-auto object-contain block mx-auto" />
                                @else
                                    <span class="text-neutral-500 font-bold text-sm sm:text-base md:text-lg lg:text-xl tracking-wider whitespace-nowrap select-none">{{ $p['name'] }}</span>
                                @endif
                            </div>
                        @endforeach
                    @else
