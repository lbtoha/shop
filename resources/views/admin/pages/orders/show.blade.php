<x-admin-app-layout>
    ...
    <div class="flex justify-between items-center gap-3 flex-wrap mb-6">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            @if ($next)
                <form action="{{ route('admin.orders.advance', $order->id) }}" method="POST" class="form-submit-edit inline">
                    @csrf @method('PUT')
                    <button type="submit" class="btn-primary !py-2">
                        <i class="ph ph-arrow-right"></i>
                        <span class="text-xs font-medium">{{ __('Mark as :status', ['status' => __($next->label())]) }}</span>
                    </button>
                </form>
            @endif
    ...
    <div class="grid grid-cols-12 gap-4 xxl:gap-6 overflow-x-hidden">
        ...
        {{-- Right: customer / shipping / payment / update --}}
