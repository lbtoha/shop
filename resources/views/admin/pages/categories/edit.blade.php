            ...
            <div class="grid grid-cols-1 gap-4 xl:gap-6">
                ...
                <div class="input-group">
                    ...
                    <x-admin::select-option id="show_in_slider" name="show_in_slider">
                        <option value="2" {{ ($category->show_in_slider ?? 2) == 2 ? 'selected' : '' }}>{{ __('Auto (Show only when it has products)') }}</option>
                        <option value="1" {{ ($category->show_in_slider ?? 2) == 1 ? 'selected' : '' }}>{{ __('Always Show (Show even if empty)') }}</option>
                        <option value="0" {{ ($category->show_in_slider ?? 2) == 0 ? 'selected' : '' }}>{{ __('Always Hide') }}</option>
                    </x-admin::select-option>
                </div>
            </div>
