<x-admin-app-layout>
    <div class="white-box">
        ...
        <x-admin::modal modalId="tiktok_pixel" title="Edit TikTok Pixel & Conversions API Credentials">
            <form class="form-submit-edit" method="POST">
                @csrf
                <div class="space-y-4">
                    <x-admin::text-input-group name="pixel_id" label="Pixel Id" />
                    <x-admin::textarea-group name="access_token" label="Conversions API Access Token" placeholder="Enter TikTok Events API Access Token" />
                    <x-admin::text-input-group name="test_event_code" label="Test Event Code (Optional)" placeholder="e.g. TEST12345 (used to verify Events API in real time)" />
                </div>
                <div class="flex items-center justify-end mt-4">
                    <x-admin::primary-button>
                        {{ __('Save') }}
