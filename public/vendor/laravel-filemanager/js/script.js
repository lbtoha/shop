...
function toBoolean(value) {
    if (value === false || value === 0 || value === "0" || value === "" || value === "false") return false;
...
function toggleActions() {
    ...
    const multiple = urlParams.get("multiple");
    if (toBoolean(multiple)) {
        $("#multi_selection_toggle").toggleClass("d-none", usingWysiwygEditor() || !many_selected);
    } else {
        $("#multi_selection_toggle").addClass("d-none");
    }
...
function preview(items) {
    ...
    items.forEach(function (item, index) {
        ...
        carouselItem.find(".carousel-level").attr("target", "_blank").attr("href", item.url).text(item.name).append($('<i class="fas fa-external-link-alt ml-2"></i>'));
